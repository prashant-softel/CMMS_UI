...
class MisTaskController extends GetxController {
  MisTaskController(
    this.misTaskPresenter,
  );
  MisTaskPresenter misTaskPresenter;
  ...
  Future<void> getAuditPlanList(int facilityId, dynamic startDate,
    ...
    // pmPlanList?.clear();
    final _auditPlanList = await misTaskPresenter.getAuditPlanList(
  ...
  Future<void> setType() async {
    try {
      // Read jobId
      String? _type = await misTaskPresenter.getValue();
      if (_type == null || _type == '' || _type == "null") {
        var dataFromPreviousScreen = Get.arguments;

        type.value = dataFromPreviousScreen['type'];
        misTaskPresenter.saveValue(type: type.value.toString());
      } else {
  ...
  Future<void> clearValue() async {
    misTaskPresenter.clearValue();
  }

  void clearStoreIdData() {
    misTaskPresenter.clearStoreIdData();
  }
}
