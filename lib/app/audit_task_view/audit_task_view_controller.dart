...
class AuditTaskViewController extends GetxController {
  ///
  AuditTaskViewController(
    this.auditTaskViewPresenter,
  );
  AuditTaskViewPresenter auditTaskViewPresenter;
  ...
  Future<void> setScheduleId() async {
    try {
      final _scheduleId = await auditTaskViewPresenter.getValue();
      ...
      if (_scheduleId == null || _scheduleId == '' || _scheduleId == "null") {
        ...
        auditTaskViewPresenter.saveValue(pmTaskId: scheduleId.value.toString());
  ...
  Future<void> getMrsListByModuleTask({required int taskId}) async {
    listMrsByTaskId?.value =
        await auditTaskViewPresenter.getMrsListByModuleTask(
              taskId,
              false,
            ) ??
            [];
  ...
  Future<void> getHistory() async {
    ...
    int moduleType = 27;
    //
    historyList?.value = await auditTaskViewPresenter.getHistory(
          // tempModuleType,
          // tempJobCardId,
          moduleType,
          scheduleId.value,
  ...
  Future<void> getAssignedToList(_facilityId) async {
    final _assignedToList =
        await auditTaskViewPresenter.getAssignedToList(facilityId: _facilityId);
...
//scheduleId = 5326;
    final _permitDetails = await auditTaskViewPresenter.getPmtaskViewList(
        scheduleId: scheduleId, isloading: isloading);
  ...
  Future<List<NewPermitModel?>?> getPermitList() async {
    ...
    final _permitList = await auditTaskViewPresenter.getPermitList(
  ...
  //       await auditTaskViewPresenter.scheduleLinkToPermit(
  ...
  void setPmTask() async {
    Map<String, dynamic>? responseMapStart =
        await auditTaskViewPresenter.setPmTask(
      scheduleId: scheduleId.value,
      isLoading: false,
    );
  ...
  approvePmTaskExecution() async {
    {
      ...
      final response = await auditTaskViewPresenter.approvePmTaskExecution(
  ...
  closePmTaskExecution() async {
    {
      ...
      final response = await auditTaskViewPresenter.closePmTaskExecution(
  ...
  rejectPmTaskExecution() async {
    {
      ...
      final response = await auditTaskViewPresenter.rejectPmTaskExecution(
  ...
  void clearMrsStoreData() {
    auditTaskViewPresenter.clearValue();
  }

  void clearStoreData() {
    auditTaskViewPresenter.clearStoreData();
  }

  void clearTypeStoreData() {
    auditTaskViewPresenter.clearTypeValue();
  }

  void clearisCheckedtoreData() {
    auditTaskViewPresenter.clearisCheckedValue();
  }

  void clearjobmodelValue() {
    auditTaskViewPresenter.clearjobmodelValue();
  }

  void clearpmTaskValue() {
    auditTaskViewPresenter.clearpmTaskValue();
  }

  void clearStoreTaskData() {
    auditTaskViewPresenter.clearStoreTaskData();
  }

  void clearStoreTaskActivityData() {
    auditTaskViewPresenter.clearStoreTaskActivityData();
  }

  void clearStoreTaskfromActorData() {
    auditTaskViewPresenter.clearStoreTaskfromActorData();
  }

  void clearStoreTasktoActorData() {
    auditTaskViewPresenter.clearStoreTasktoActorData();
  }

  void clearStoreTaskWhereUsedData() {
    auditTaskViewPresenter.clearStoreTaskWhereUsedData();
  }
}
