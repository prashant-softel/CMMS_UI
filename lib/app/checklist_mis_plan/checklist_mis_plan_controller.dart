...
class ChecklistMisPlanController extends GetxController {
  ChecklistMisPlanController(
    this.checklistMisPlanPresenter,
  );
  ChecklistMisPlanPresenter checklistMisPlanPresenter;
  ...
  Future<void> getAuditPlanList(int facilityId, dynamic startDate,
    ...
    // pmPlanList?.clear();
    final _auditPlanList = await checklistMisPlanPresenter.getAuditPlanList(
  ...
  Future<void> setType() async {
    try {
      // Read jobId
      String? _type = await checklistMisPlanPresenter.getValue();
      if (_type == null || _type == '' || _type == "null") {
        var dataFromPreviousScreen = Get.arguments;

        type.value = dataFromPreviousScreen['type'];
        checklistMisPlanPresenter.saveValue(type: type.value.toString());
      } else {
  ...
  Future<void> clearValue() async {
    checklistMisPlanPresenter.clearValue();
  }

  void clearStoreIdData() {
    checklistMisPlanPresenter.clearStoreIdData();
  }
}
