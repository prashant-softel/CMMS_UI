...
class ObservationSummaryController extends GetxController {
  ///
  ObservationSummaryController(
    this.observationSummaryPresenter,
  );
  ObservationSummaryPresenter observationSummaryPresenter;
  ...
  RxList<ObservationSummaryModel?> observationSummaryList =
      <ObservationSummaryModel?>[].obs;
  RxList<ObservationSummaryModel?> filteredData =
      <ObservationSummaryModel>[].obs;
  ...
  void onInit() async {
    facilityIdStreamSubscription = controller.facilityId$.listen((event) {
      ...
      Future.delayed(Duration(seconds: 1), () {
        getObservationSummary(facilityId, formattedFromdate, formattedTodate);
      });
    });
    super.onInit();
  }

  Future<void> getObservationSummary(
      int facilityId, dynamic fromDate, dynamic toDate) async {
    observationSummaryList.value = <ObservationSummaryModel>[];
    final _observationSummaryList =
        await observationSummaryPresenter.observationSummaryData(
    ...
    if (_observationSummaryList != null) {
      observationSummaryList.value = _observationSummaryList;
    }
    isLoading.value = false;
    update(['observation_summary_list']);
  }

  void getObservationSummaryByDate() {
    getObservationSummary(facilityId, formattedFromdate1, formattedTodate1);
  }
}
