...
import '../../domain/models/create_unitOfMeasurement_model.dart';
...
class UnitOfMeasurementController extends GetxController {
  ...
  RxList<UnitOfMeasurementModel?>? moduleList = <UnitOfMeasurementModel?>[].obs;
  int facilityId = 0;
  int type = 1;
  ...
  Future<bool> createModuleListNumber() async {
     print("For API testing controller entry");
    if (nameCtrlr.text.trim() == '') {
      Fluttertoast.showToast(
          msg: "Please enter required field", fontSize: 16.0);
          
    } else {
      String _moduleListNumber = nameCtrlr.text.trim();

      CreateUnitOfMeasurementModel createModuleList =
          CreateUnitOfMeasurementModel(
        name: _moduleListNumber,
        decimal_status: isToggle1On.value ? 1 : 0,
        spare_multi_selection: isToggle2On.value ? 1 : 0,
      );
      ...
      await unitOfMeasurementPresenter.createUnitOfMeasurement(
        modulelistJsonString: moduleListJsonString,
        isLoading: true,
      );
     print("For API testing checklist json string exit");

      return true;
    }
    getUnitOfMeasurementList(facilityId, type, true);
     print("For API testing controller exit");
